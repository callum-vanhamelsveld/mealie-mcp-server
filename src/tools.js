...
export const tools = {
    ...
    handler: async (args) => {
      ...
      try {
        ...
        // Log succinct tool error to stderr so clients can display it
        try {
          console.error(
            JSON.stringify({
              type: "tool_error",
  ...
  get_recipe_by_id: {
    ...
    handler: async (args) => {
      ...
      const { id, slug } = args;
      const key = id || slug; // accept either

      try {
        // Mealie detail: GET /api/recipes/{slug} where slug can be the slug or UUID id
        const res = await http.get(`/api/recipes/${key}`);
        const data = res.data;

        const ingredients =
          data.recipeIngredient ||
          (data.ingredients || []).map(
            (i) => i.note || i.text || i.name || ""
          ) ||
          [];
        ...
        const instructions = data.recipeInstructions
          ...
          : (data.steps || data.instructions || []).map(
              (s) => s.text || s.step || ""
            );

        const tags = (data.tags || data.recipeCategory || [])
          .map((t) => {
        ...
        return {
          ...
          tags,
          slug: data.slug || slug || "",
          url: `${http.defaults.baseURL.replace(/\/+$/, "")}/recipe/${data.slug || key}`
        };
      } catch (err) {
        const msg = normalizeError(err);
        // Log succinct tool error to stderr so clients can display it
